...
```
* 值初始化，这是在以空初始化器构造对象时进行的初始化。
```
 std::string s{};
```
* 直接初始化，从明确的构造函数实参的集合初始化对象。
```
std::string s("hello");
```
* 复制初始化，从另一对象初始化对象。
...
std::string s = "hello";
```
* 列表初始化，从花括号初始化器列表初始化对象。
```
std::string s{'a', 'b', 'c'};
```
* 聚合初始化，从花括号初始化器列表初始化聚合体。
```
char a[3] = {'a', 'b'};
```
* 引用初始化，绑定引用到对象。
...
+ [global-variable](https://github.com/WCIofQMandRA/global-variable)
